...
namespace MyLib
{
    public class BasicDataIndexMember
    {
        public readonly int IDo;
        public readonly int IXi;
        public readonly int s;
        string[] checks = { "IDo", "IXi", "s" };

        public BasicDataIndexMember(string name)
        {
            ...
            foreach (string word in words)
            {
                foreach (string check in checks)
                {
                    if (word.StartsWith(checks[0]))
                        ...
                        IXi = Int32.Parse(word.Substring(3, word.Length - 3));
